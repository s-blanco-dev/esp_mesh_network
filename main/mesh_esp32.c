...
#define CONFIG_MESH_ROUTER_SSID "Bola_de_fraile_2.4"
#define CONFIG_MESH_ROUTER_PASSWD "487957817"
...
/* Tarea handles para evitar crear múltiples instancias */
...
void send_task(void *arg)
{
    ...
    while (1) {
        if (!esp_mesh_is_root()) {
            /* obtiene la MAC del AP del parent */
            esp_mesh_get_parent_bssid(&parent);

            /* rellena self_mac */
            esp_wifi_get_mac(WIFI_IF_STA, pkt.self_mac);

            /* calcula la MAC del STA del parent restando 1 (con borrow seguro) */
            ...
            if (err == ESP_OK) {
                ESP_LOGI(TAG, "Sent packet to root (layer %d)", pkt.layer);
            } else {
...
void receive_task(void *arg)
{
    ...
    while (1) {
        ...
        if (err == ESP_OK && data.size == sizeof(node_packet_t)) {
            ...
            ESP_LOGI(TAG, "Packet received from %02x:%02x:%02x:%02x:%02x:%02x",
                     from.addr[0], from.addr[1], from.addr[2],
                     from.addr[3], from.addr[4], from.addr[5]);

            ESP_LOGI(TAG, "Self MAC: %02x:%02x:%02x:%02x:%02x:%02x | Parent MAC: %02x:%02x:%02x:%02x:%02x:%02x | Layer: %d | Temp: %d Humidity: %d",
            ...
            uart_write_bytes(UART_NUM, (char *)pkt, sizeof(node_packet_t));
        }
    }
}
...
static void init_uart()
{
    ...
    ESP_LOGI(TAG, "UART1 marchando para transmitir al ESP receptor");
}
                               ...
                               int32_t event_id, void *event_data)
{
    ...
    switch (event_id) {
        ...
        case MESH_EVENT_CHILD_CONNECTED:
            /* No llamar a receive_task() directamente: crear la tarea si hace falta.
               Normalmente startTask() ya la habrá creado cuando el nodo se convirtió en root,
               pero en caso de que el root reciba un child mientras corre, asegúrate de tener la tarea. */
        ...
        case MESH_EVENT_PARENT_CONNECTED:
            ...
            /* Asegúrate de tener la tarea de envío en el hijo */
        ...
        case MESH_EVENT_PARENT_DISCONNECTED:
            ...
            /* puedes decidir borrar send_task_handle si quieres que se recree al reconectar */
            break;
...
void app_main(void)
{
    ...
    /* Aumentar este tiempo puede ayudar a evitar auth/beacon timeouts en condiciones inestables */
